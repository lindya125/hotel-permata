...
class Admin extends CI_Controller
{
    ...
    public function index()
    {
        ...
        //mengupdate stok dan dibooking pada tabel kamar         
        ...
        $detail = $this->db->query("SELECT*FROM booking,booking_detail,kamar WHERE booking.id_booking=booking_detail.id_booking AND booking_detail.id_kamar=kamar.id")->result_array();
        foreach ($detail as $key) {
            $id_kamar     = $key['id_kamar'];
            $batas_ambil = $key['tgl_booking'];
            $tglawal     = date_create($batas_ambil);
            $tglskrg     = date_create();
            $beda        = date_diff($tglawal, $tglskrg);

            if ($beda->days > 2) {
                $this->db->query("UPDATE kamar SET stok=stok+1, check_in=check_in-1 WHERE id='$id_kamar'");
            }
        }
